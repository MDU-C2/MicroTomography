function [MeshVerts,MeshEdges,MedialAxis,MAT,verts] = PowerCrust(points,optionToPlot)
    ...
    %% Step 2: Find Poles
    disp('Finding Poles');
    [poleVerts, poleRadMat, sampleIdxForPole, oppositePoleIdx] = FindPoles( verts, cells, points,optionToPlot);

    %% Step 3: Compute Power Diagram of Poles
    disp('Finding Power Diagram');
    [PD, ~] = powerDiagramWrapper(poleVerts, poleRadMat .^2);

    %% Step 4: Label Poles as either inside or outside the point cloud (1 or 0)
    disp('Labeling Poles');
    ...
    DisplayMedialAxis(MedialAxis,MAT,points);
    MeshEdges = cell2mat(MeshEdges);
end