...
def move_to_desired_location_upwards(desired_distance_mm):  # Get distance from app.py
    ...
    steps_to_move = calculate_steps(
        desired_distance_mm
    )  # calculating the desired distance in steps

    total_steps_changes = move(
        steps_to_move, total_steps_changes
...
def move_to_zeroLocation(
    total_steps_changes,
):  # This function move the motor to zero location
