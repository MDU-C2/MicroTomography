...
%% Remove nan values
%NanIndexes = ~all(isnan(data), 2);
%NanIndexes = ~isnan(data);
...
%indx_z = indx_z-1;
z_min = min(min(data(:,3,:)));
z_max = max(max(data(:,3,:)));
...
for i = 1:size(data,3)
    ...
    x_spline = smooth_spline(z,x,show_fit);
    y_spline = smooth_spline(z, y, show_fit);
