...
for i = 1:size(data,3)
    ...
    % INTERPOLATION 
    % When the azimuth angle is 0 , 90, 180 , 270 the data in X or Y
    % direction is almost linear ( in this case seconde polynomial is used.
    % This is hard coded now to a certain index 
    % Make the code by calculating by calculating the index of 
    % 1, 11, 6, 16 automaticaly. 
    switch i
        % For 0 and 180 azimuth Y is almost linear
        % i = 1 and i = 11 are poly interpolation for (Z,Y).
    case i == 1 || i ==11
        x_spline = smooth_spline_zx(z,x,show_fit);
        y_spline = poly2_fit(z, y, show_fit);
        % For 90 and 270 azimuth X is almost linear
        % i = 6 and i = 16 are poly interpolation for (Z,X).
    case i == 6 || i == 16
        x_spline = poly2_fit(z,x,false);
        y_spline = smooth_spline_zx(z, y, show_fit);
        % All other angles are smooth spline interpolated
    otherwise
        x_spline = smooth_spline_zx(z,x,show_fit);
        y_spline = smooth_spline_zx(z, y, show_fit);
    end
